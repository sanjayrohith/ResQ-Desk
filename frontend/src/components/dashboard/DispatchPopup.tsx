...
export function DispatchPopup({ data, onCancel, onComplete }: DispatchPopupProps) {
  ...
  // After a successful reallocation confirm, auto-close.
  useEffect(() => {
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full transition-all duration-500 ${
          dispatched
            ...
            : isReallocation
              ? 'bg-amber-500/20 blur-[100px] animate-pulse'
              ...
              <>
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-xl border ${isReallocation ? 'bg-amber-500/20 border-amber-500/30' : 'bg-red-500/20 border-red-500/30'}`}>
                      {isReallocation
                        ? <Repeat className="w-5 h-5 text-amber-400 animate-pulse" />
                        : <Siren className="w-5 h-5 text-red-400 animate-pulse" />}
                    </div>
                    <div>
                      <h2 className="text-lg font-bold text-white">{isReallocation ? "Reallocation Required" : "Auto-Dispatch"}</h2>
                      <p className="text-xs text-slate-500">{isReallocation ? "Operator confirmation needed" : "Initiating unit deployment"}</p>
                    </div>
                  </div>
                  <button
                    onClick={onCancel}
                ...
                {/* Reallocation warning banner */}
                {isReallocation && (
                  <div className="mb-6 rounded-2xl bg-amber-500/10 border border-amber-500/30 p-4 flex items-start gap-3">
                    <AlertTriangle className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
                    <div>
                      <div className="text-xs font-bold text-amber-400 uppercase tracking-wider mb-1">No Free Unit Available</div>
                      <p className="text-sm text-slate-300 leading-relaxed">{reallocation.message}</p>
                      {reallocation.from_incident && (
                        <p className="text-[11px] text-slate-500 mt-2">
                          Pulling from incident <span className="font-mono text-slate-400">{reallocation.from_incident}</span>
                          {reallocation.from_severity ? ` · ${reallocation.from_severity}` : ""}
                        </p>
                      )}
                    </div>
                  </div>
                ...
                {/* Countdown Display (auto-dispatch only) */}
                {!isReallocation && (
                ...
                <div className="rounded-2xl bg-slate-900/50 border border-slate-700/50 p-5 mb-6 relative overflow-hidden">
                  ...
                  <div className="space-y-4 relative z-10">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-[10px] text-slate-500 uppercase tracking-wider font-medium mb-1">{isReallocation ? "Reallocating Unit" : "Assigning Unit"}</div>
                        <div className="flex items-center gap-3">
                      ...
                      <div className={`p-3 rounded-xl border ${isReallocation ? 'bg-amber-500/10 border-amber-500/20' : 'bg-cyan-500/10 border-cyan-500/20'}`}>
                        <Navigation className={`w-6 h-6 ${isReallocation ? 'text-amber-400' : 'text-cyan-400'}`} />
                      </div>
                    </div>
                ...
                {/* Progress Bar (auto-dispatch only) */}
                {!isReallocation && (
                  <div className="mb-6">
                ...
                {isReallocation ? (
                  <div className="flex gap-3">
                    <button
                      onClick={onCancel}
                      disabled={confirming}
                      className="flex-1 py-4 rounded-xl font-semibold text-sm uppercase tracking-wider transition-all flex items-center justify-center gap-2 bg-slate-800/50 border border-slate-700/50 text-slate-400 hover:bg-slate-800 disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      disabled={confirming}
                      className="flex-[1.5] py-4 rounded-xl font-semibold text-sm uppercase tracking-wider transition-all flex items-center justify-center gap-2 bg-amber-500/15 border border-amber-500/40 text-amber-300 hover:bg-amber-500/25 disabled:opacity-50"
                    >
                      <Repeat className={`w-4 h-4 ${confirming ? 'animate-spin' : ''}`} />
                      {confirming ? "Reallocating..." : "Confirm Reallocation"}
                    </button>
                  </div>
                ) : (
                  <button
